...
server.tool(
  ...
  async ({ componentName }) => {
    try {
      ...
      const message =
        (e instanceof Error ? e.message : e) || `Failed to get metadata${componentName ? ` for ${componentName}` : ''}`;
      return {
...
server.tool(
  ...
  "Returns a list of available Vibe components",
  {},
  async ({}) => {
    try {
...
server.tool(
  "getComponentContext",
  "Return context for a given component for the LLM to use to write code",
  {componentName: z.string().describe('The name of the component to get code samples for')},
  async ({componentName}) => {
    try {
